// localStorage.setItem('otherUserName', 'rift');
// localStorage.setItem('otherUserID', 8);

var otherUsername = localStorage.getItem('otherUserName');
let otherUserID = localStorage.getItem('otherUserID');

const ownUserName = localStorage.getItem('userNameLocal');
const ownUserID = localStorage.getItem('ownUserID');

document.addEventListener('DOMContentLoaded', function () {
    fetch('http://localhost:5000/gettingCurrentUserInfo/' + otherUsername)
        .then(response => response.json())
        .then(data => loadUserProfileWithCurrentUser(data['data']));

    fetch('http://localhost:5000/gettingFriendsInfo/' + otherUserID)
        .then(response => response.json())
        .then(data => set_friends(data['data']));

    fetch('http://localhost:5000/gettingFollowersInfo/' + otherUserID)
        .then(response => response.json())
        .then(data => set_followers(data['data']));

    fetch('http://localhost:5000/gettingFollowingsInfo/' + otherUserID)
        .then(response => response.json())
        .then(data => set_followings(data['data']));


    // About info
    fetch('http://localhost:5000/getAllWorkData/' + otherUserID)
        .then(response => response.json())
        .then(data => loadWorkData(data['data']));

    fetch('http://localhost:5000/getAllEduData/' + otherUserID)
        .then(response => response.json())
        .then(data => loadEduData(data['data']));

    fetch('http://localhost:5000/getAllAddressData/' + otherUserID)
        .then(response => response.json())
        .then(data => loadAddressData(data['data']));

    fetch('http://localhost:5000/getAllLinksData/' + otherUserID)
        .then(response => response.json())
        .then(data => loadLinksData(data['data']));

    // Checking friend and follower

    fetch(`http://localhost:5000/searchingFriendStatus/${ownUserID}/${otherUserID}`)
        .then(response => response.json())
        .then(data => toSeeFriends(data['data']));

    fetch(`http://localhost:5000/searchingFriendStatus1/${ownUserID}/${otherUserID}`)
        .then(response => response.json())
        .then(data => toSeePending(data['data']));

    fetch(`http://localhost:5000/searchingFriendStatus2/${ownUserID}/${otherUserID}`)
        .then(response => response.json())
        .then(data => toSeeAccept(data['data']));

    fetch(`http://localhost:5000/gettingMutualFriend/${ownUserID}/${otherUserID}`)
        .then(response => response.json())
        .then(data => set_mutual_friends(data['data']));
})
...
function set_mutual_friends(data) {
    var count = data.length;
    document.getElementById('mutual-friends-count').innerHTML = count;
}

function toSeeFriends(data) {
    if (data.length != 0) {
        document.getElementById('friend-btn-id').innerHTML = 'Friends'
    }
}
function toSeePending(data) {
    if (data.length != 0) {
        document.getElementById('friend-btn-id').innerHTML = 'Pending'
    }
}
function toSeeAccept(data) {
    if (data.length != 0) {
        document.getElementById('friend-btn-id').innerHTML = 'Accept'
    }
}
...
const friend_btn_id = document.getElementById('friend-btn-id');
friend_btn_id.addEventListener('click', function () {
    const text = friend_btn_id.innerHTML;
    if (text === 'Add Friend') {
        fetch('http://localhost:5000/insertingFriendReq', {
            headers: {
                'Content-type': 'application/json'
            },
            method: 'POST',
            body: JSON.stringify({ ownUserID: ownUserID, otherUserID: otherUserID })
        })
            .then(response => response.json())
            .then(data => friend_btn_id.innerHTML = 'Pending');
        // Notification

        sendFriendRelatedNotification(1);
    }
    else if (text === 'Pending' || text === 'Friends') {
        fetch(`http://localhost:5000/cancelFriendReq/${ownUserID}/${otherUserID}`, {
            method: 'DELETE'
        })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    friend_btn_id.innerHTML = 'Add Friend'
                    location.reload()
                }
            });
    }
        ...
        .then(data => {
            if(data.success){
                friend_btn_id.innerHTML = 'Friends'

                // Notify
                sendFriendRelatedNotification(2);                
            }
        })
    }
})
...
function sendFriendRelatedNotification(x){
    let id = 0;
    if(x == 1)
        id = ownUserID;
    else if(x == 2){
    ...
    fetch('http://localhost:5000/gettingNameofSender/' + id)
        .then(response => response.json())
        .then(data => sendNotification(data['data'], x));
}
function sendNotification(data, x){
...
document.addEventListener('DOMContentLoaded', function () {
    fetch('http://localhost:5000/getAllPostPerUser/'+otherUserID)
        .then(response => response.json())
